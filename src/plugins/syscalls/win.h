...
public:
    ...
    void print_syscall(drakvuf_t drakvuf, drakvuf_trap_info_t* info, int nr, const char* module, const syscalls_ns::syscall_t* sc, std::vector<uint64_t> args, privilege_mode_t mode, std::optional<syscalls_ns::syscalls_module> from_dll, std::optional<syscalls_ns::syscalls_module> from_parent_dll);

    win_syscalls(drakvuf_t drakvuf, const syscalls_config* config);
    ~win_syscalls();
};
